...
interface AdminVideoFilterBarProps {
  ...
  countsScope?: "admin" | "admin-video-tests";
}
...
export function AdminVideoFilterBar({
  filters,
  onFiltersChange,
  countsScope = "admin",
      ...
      <div className="flex flex-wrap items-end gap-4">
        ...
        <div className="space-y-1.5">
          ...
          <SegmentedControl
            ...
            onChange={(val) => onFiltersChange({ ...filters, usageStatus: val as any })}
...
function FilterDropdown({
  ...
  const options = rawOptions.filter((opt: Tag) => {
    ...
    // If counts are unavailable, avoid hiding all options
    if (Object.keys(counts).length === 0) return true;
    // Category DDM: only show options with >0 matching videos
    if (isCategoryFilter) return count > 0;
    // Other DDMs: when categories selected, only show options with >0
